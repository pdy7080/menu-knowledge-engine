...
async def translate_with_claude_cli(
    menu_name_ko: str, description_en: str
...
{{"ja": "일본어 번역", "zh": "중국어(간체) 번역"}}"""
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(
                f"Claude CLI error (exit {result.returncode}): {result.stderr}"
            )
            return None
        ...
        return {"ja": data.get("ja", ""), "zh": data.get("zh", "")}
        ...
        logger.error(
            f"JSON parse error for {menu_name_ko}: {e}\nOutput: {result.stdout}"
        )
        return None
...
async def get_untranslated_menus(db: AsyncSession) -> list:
    """미번역 메뉴 전체 조회"""
    query = select(CanonicalMenu).where(CanonicalMenu.name_ja == None)
...
async def main():
    ...
    async with async_session() as db:
        ...
        # 번역 실행
        results = {"success": 0, "failed": 0, "skipped": 0, "db_error": 0}
