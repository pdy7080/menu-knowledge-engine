...
async def main():
    ...
    print("  가장 많은 메뉴 (전국 인기):")
    for name, count in food_counter.most_common(15):
        detail = food_details[name]
        avg_price = (
            sum(detail["prices"]) // len(detail["prices"]) if detail["prices"] else 0
        )
        ...
        if detail["is_premium"]:
            flags += "[PREM]"
        print(
            f"    {name:<20} {count:3d}곳 | avg {avg_price:>6,}원 | {rest_count:3d}개 휴게소 {flags}"
        )
    ...
    for name in food_counter:
        if any(
            k in name for k in ["우동", "국수", "라면", "면", "소바", "파스타", "냉면"]
        ):
            categories["면류"] += 1
    ...
    for name, count in food_counter.most_common():
        ...
        if rest_count >= 3 and len(name) >= 2:
            avg_price = (
                sum(detail["prices"]) // len(detail["prices"])
                if detail["prices"]
                else 0
            )
    ...
    print(
        f"  3곳 이상 판매 메뉴: {len(canonical_candidates)}개 (전체 {unique_foods}개 중)"
    )
    ...
    unique_list = [
        {
            ...
            "avg_price": (
                sum(food_details[name]["prices"]) // len(food_details[name]["prices"])
                if food_details[name]["prices"]
                else 0
            ),
        }
        for name, count in food_counter.most_common()
