...
async def main():
    ...
    print("모델: gpt-4o-mini (temperature=0.3)")
    print("예상 비용: ~$0.05")
    print("=" * 60)
    ...
    for r in results:
        ...
        checks.append(
            f"[{'OK' if 150 <= desc_ko_len <= 250 else 'WARN'}] KO desc: {desc_ko_len} chars"
        )
        checks.append(
            f"[{'OK' if 150 <= desc_en_len <= 250 else 'WARN'}] EN desc: {desc_en_len} chars"
        )
        ...
        checks.append(
            f"[{'OK' if variants_count >= 3 else 'WARN'}] Regional: {variants_count}"
        )
        checks.append(f"[{'OK' if steps_count >= 5 else 'WARN'}] Steps: {steps_count}")
        checks.append(
            f"[{'OK' if similar_count >= 3 else 'WARN'}] Similar: {similar_count}"
        )
    ...
    print("\n[OK] 테스트 완료!")
    print("\n다음 단계: 품질 검증 실행")
    print("  python app/backend/scripts/validate_enriched_content.py")
