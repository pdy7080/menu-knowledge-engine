...
class ContentEnricher:
    ...
    def _build_prompt(self, menu: CanonicalMenu) -> str:
        ...
        main_ingredients_str = (
            ", ".join([ing.get("ko", "") for ing in menu.main_ingredients])
        ...
        # 기존 설명 활용 (있는 경우)
        existing_desc_ko = (
            menu.explanation_short.get("ko", "")
            if isinstance(menu.explanation_short, dict)
            else ""
        )
        existing_desc_en = (
            menu.explanation_short.get("en", "")
            if isinstance(menu.explanation_short, dict)
            else ""
        )
...
async def main():
    ...
    print("\n✅ 모든 작업 완료!")
    print("다음 단계: 품질 검증 (data/enriched_menus.json 확인)")
