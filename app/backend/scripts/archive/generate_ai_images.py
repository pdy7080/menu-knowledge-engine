...
def generate_dalle3_prompt(
    dish_name_ko: str, dish_name_en: str, description: str
) -> str:
    """
...
def generate_korean_food_images(
    ...
    print("Cost per image: $0.08 (1024×1024, standard quality)")
    ...
    for i, (name_ko, name_en, description) in enumerate(dishes_to_generate, 1):
        ...
        # Generate image
        print("  [GEN] Calling DALL-E 3 API...")
        ...
        if result["success"]:
            ...
            total_cost += 0.08
            print(
                f"  [OK] Generated successfully (Cost: $0.08, Total: ${total_cost:.2f})"
            )
        ...
        if i < count:
            print("  [WAIT] Cooling down (rate limit)...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate Korean food images using DALL-E 3"
    )
    parser.add_argument(
        "--count", type=int, default=60, help="Number of images to generate"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="C:/project/menu/data/images/ai_generated",
