...
def main():
    ...
    canonical_list = [
        (name, data) for name, data in sorted_menus if data["rest_area_count"] >= 5
    ]
...
"""
    ...
    for e in canonicals[:20]:
        print(
            f"    {e['name_ko']:<20} {e['rest_area_count']:3d}곳 | avg {e['avg_price']:>6,}원 | {e['category_1']}/{e['category_2']}"
        )
    ...
    for e in variants[:15]:
        ...
        print(
            f"    {e['name_ko']:<20} = [{mods_str}] + {e['base_name']} | {e['rest_area_count']:3d}곳"
        )
