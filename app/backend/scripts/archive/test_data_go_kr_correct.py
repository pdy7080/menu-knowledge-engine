...
async def main():
    ...
    print("URL: apis.data.go.kr/1471000/FoodNtrCpntDbInfo02/getFoodNtrCpntDbInq02")
    print("=" * 60)
                    ...
                    if "header" in data:
                        ...
                        if "body" in data:
                            ...
                            if isinstance(items, list):
                                for item in items[:2]:
                                    ...
                                    energy = item.get(
                                        "AMT_NUM1", item.get("ENERC", "N/A")
                                    )
                                    protein = item.get(
                                        "AMT_NUM3", item.get("PROT", "N/A")
                                    )
                                    cat1 = item.get("FOOD_CAT1_NM", "N/A")
                                    print(
                                        f"  -> {name} | {cat1} | energy={energy} protein={protein}"
                                    )
                        ...
                        if isinstance(items, list):
                            for item in items[:2]:
                                ...
                                name = item.get(
                                    "FOOD_NM_KR", item.get("foodNmKr", "N/A")
                                )
                                print(f"  -> {name}")
