                ...
                if status == 200:
                    try:
                        ...
                        if "data" in data or "currentCount" in data:
                            ...
                            print(
                                f"    응답: {json.dumps(data, ensure_ascii=False, indent=2)[:400]}"
                            )
                            return url, data
                        elif "resultCode" in str(data):
                            code = data.get(
                                "resultCode",
                                data.get("header", {}).get("resultCode", "N/A"),
                            )
                            print(
                                f"    패턴{idx} HTTP 200 but code={code}: {text[:150]}"
                            )
                        else:
                        ...
                        if "SERVICE_KEY" in text or "SERVICE" in text:
                            print(
                                f"    패턴{idx} HTTP {status}: 키 관련 오류 - {text[:150]}"
                            )
                        else:
