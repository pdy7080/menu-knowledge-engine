        ...
        if status == 200:
            try:
                ...
                if "list" in parsed:
                    ...
                    for item in items[:3]:
                        print(
                            f"    - {item.get('unitName', 'N/A')} ({item.get('routeName', 'N/A')})"
                        )
                elif "count" in parsed:
                    print(f"  총 건수: {parsed.get('count')}")
                else:
                    print(
                        f"  데이터: {json.dumps(parsed, ensure_ascii=False, indent=2)[:500]}"
                    )
        ...
        if status == 200:
            try:
                ...
                if "list" in parsed:
                    ...
                    for item in items[:5]:
                        print(
                            f"    - {item.get('foodNm', 'N/A')} | {item.get('unitNm', 'N/A')} | {item.get('foodCost', 'N/A')}원"
                        )
                elif "count" in parsed:
                    print(f"  총 건수: {parsed.get('count')}")
                else:
            ...
            try:
                ...
                if "list" in parsed:
                    ...
                    if "list" not in parsed:
                        print(
                            f"  전체 응답: {json.dumps(parsed, ensure_ascii=False, indent=2)[:800]}"
                        )
                else:
                    print(
                        f"  데이터: {json.dumps(parsed, ensure_ascii=False, indent=2)[:800]}"
                    )
            except json.JSONDecodeError:
                print(f"  응답(raw): {data[:500]}")
        else:
