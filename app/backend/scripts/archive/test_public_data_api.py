...
async def main():
    ...
    print(
        f"API 키: {API_KEY[:4]}...{API_KEY[-4:]}"
        if len(API_KEY) > 8
        else f"API 키: {API_KEY}"
    )
    print("=" * 60)
        ...
        try:
            ...
            try:
                ...
                if "header" in parsed:
                    ...
                    print(
                        f"  {menu}: HTTP {status} | code={result_code} | msg={result_msg}"
                    )
                    ...
                    print(
                        f"  {menu}: HTTP {status} | code={result_code} | total={total}"
                    )
                    if total > 0:
        ...
        try:
            ...
            try:
                ...
                if "header" in parsed:
                    header = parsed["header"]
                    print(
                        f"  {menu}: HTTP {status} | code={header.get('resultCode')} | msg={header.get('resultMsg')}"
                    )
                elif "response" in parsed:
                    header = parsed["response"].get("header", {})
                    body = parsed["response"].get("body", {})
                    total = body.get("totalCount", 0)
                    print(
                        f"  {menu}: HTTP {status} | code={header.get('resultCode')} | total={total}"
                    )
                    if total > 0:
                        items = body.get("items", {})
                        if isinstance(items, dict):
        ...
        try:
            ...
            try:
                ...
                if "SERVICE_KEY_IS_NOT_REGISTERED_ERROR" in data:
                    print(
                        f"  {menu}: HTTP {status} | API 키 미등록 (이 API에 별도 신청 필요)"
                    )
                elif "UNREGISTERED_SERVICE" in data:
                    print(
                        f"  {menu}: HTTP {status} | 서비스 미등록 (이 API에 별도 신청 필요)"
                    )
                else:
