...
"""
        ...
        for i in range(0, len(menus), batch_size):
            ...
            tasks = [
                self.translate_menu_description(
                    menu["name_ko"], menu["description_en"], target_languages
                )
                for menu in batch
            ]
    ...
    def save_to_database(self, menu_id: str, translations: Dict[str, str]):
        ...
        with Session(self.engine) as session:
            menu = (
                session.query(CanonicalMenu).filter(CanonicalMenu.id == menu_id).first()
            )

            if menu:
                # JSONB 컬럼 업데이트 (SQLAlchemy 변경 감지를 위해 전체 재할당)
                updated_dict = (
                    menu.explanation_short.copy() if menu.explanation_short else {}
                )
...
async def main():
    ...
    parser.add_argument(
        "--language", default="ja,zh", help="번역 언어 (쉼표로 구분, 기본: ja,zh)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="동시 처리 개수 (기본: 10)"
    )
    parser.add_argument(
        "--max-retries", type=int, default=3, help="최대 재시도 횟수 (기본: 3)"
    )
    ...
    menus_to_translate = [
        {
            ...
            "description_en": (
                menu.explanation_short.get("en", "") if menu.explanation_short else ""
            ),
        }
    ...
    print("\n" + "=" * 60)
    print("[STATS] Translation Summary")
    print("=" * 60)
    ...
    print("   Next: I18n-Auditor re-validation")
