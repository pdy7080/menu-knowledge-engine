...
MENU_SEARCH_TERMS = {
    ...
    "소고기국밥": "sogogi gukbap",
    "콩나물국밥": "kongnamul gukbap",
    "해장국": "haejangguk",
    # 추가 메뉴 (커버리지 확대)
    "소떡소떡": "sotteok sotteok",
    "냉모밀": "soba noodles cold",
    "통감자": "whole potato korean",
    "장터국밥": "gukbap korean rice soup",
    "바삭어포": "dried fish snack korean",
    "강원나물밥": "namul bap bibimbap",
    "해물순두부": "sundubu jjigae",
    "해물바": "seafood bar korean",
    "옥수수": "corn korean street food",
    "제육볶음": "jeyuk bokkeum",
    "회오리감자": "tornado potato korean",
    "열무국수": "korean noodle soup cold",
    "함박스테이크": "hamburg steak korean",
    "한우국밥": "beef gukbap korean",
    "케이크소세지": "korean sausage snack",
    "케네디소시지": "sausage korean",
    "가락자장면": "jajangmyeon",
    "사골우거지국밥": "korean beef bone soup",
    "사골우거지국": "korean beef bone soup",
    "델리만쥬": "delimanjoo korean",
    "고등어구이": "grilled mackerel korean",
    "미니츄러스": "churros korean",
    "고등어구이정식": "grilled mackerel korean",
...
def download_wiki_images(limit: int = 0) -> dict:
    ...
    for i, menu_name in enumerate(menus, 1):
        ...
        if search_term in seen_terms:
            ...
            if src_info:
                src_path, src_thumb_url = (
                    src_info if isinstance(src_info, tuple) else (src_info, None)
                )
        ...
        try:
            ...
            for retry in range(3):
                ...
                if dl_resp.status_code == 429:
                    wait = 15 * (retry + 1)
                    print(
                        f"  [{i:3d}] {menu_name}: Rate limited, waiting {wait}s... (retry {retry+1}/3)"
                    )
                    time.sleep(wait)
                    continue
                break
            ...
            if dl_resp.status_code == 200 and len(dl_resp.content) > 1000:
                ...
                if "image" in ct:
                    ...
                    print(
                        f"  [{i:3d}] {menu_name} -> {len(dl_resp.content)//1024}KB (term={search_term})"
                    )
                else:
                    failed += 1
        ...
        if i % 20 == 0 or i == len(menus):
            ...
            print(
                f"  --- [{i}/{len(menus)}] 신규 {new}개 + 복사 {copied}개 + 캐시 {cached}개, 실패 {failed}개 ---"
            )
...
def upload_to_r2(all_images: dict) -> dict:
    ...
    if not all([account_id, access_key, secret_key, bucket]):
        ...
        print(
            "  → R2 설정 시: R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY 필요"
        )
...
def main():
    ...
    parser.add_argument(
        "--download-only", action="store_true", help="다운로드만 (R2 업로드 안함)"
    )
    parser.add_argument(
        "--upload-only", action="store_true", help="로컬 이미지만 R2 업로드"
    )
    parser.add_argument("--limit", type=int, default=0, help="처리할 메뉴 수 제한")
    parser.add_argument(
        "--source", choices=["wiki", "ai", "all"], default="all", help="이미지 소스"
    )
    ...
    print(
        f"모드: {'다운로드만' if args.download_only else '업로드만' if args.upload_only else '전체'}"
    )
    print("=" * 60)
