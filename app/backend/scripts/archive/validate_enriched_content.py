...
class ContentValidator:
    ...
    def validate_menu(self, menu_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(desc_ko) < 100:
            warnings.append(
                f"한국어 설명이 너무 짧음: {len(desc_ko)}자 (권장: 150-200자)"
            )
            score -= 3
            self.warning_summary["desc_ko_short"] += 1
        elif len(desc_ko) > 250:
            warnings.append(
                f"한국어 설명이 너무 김: {len(desc_ko)}자 (권장: 150-200자)"
            )
            score -= 2
        ...
        if len(desc_en) < 100:
            warnings.append(
                f"영어 설명이 너무 짧음: {len(desc_en)}자 (권장: 150-200자)"
            )
            score -= 3
        ...
        if not isinstance(flavor, dict):
            ...
            for field in flavor_fields:
                ...
                if not isinstance(value, (int, float)) or value < 1 or value > 5:
                    warnings.append(
                        f"맛 프로필 범위 오류: {field} = {value} (1-5 필요)"
                    )
                    score -= 2
...
"""
        ...
        for warning_type, count in sorted(
            self.warning_summary.items(), key=lambda x: -x[1]
        ):
