...
class ProductionSync:
    ...
    async def _export_for_manual_sync(self) -> Dict[str, Any]:
        ...
        enriched_dir = self.staging_dir / "enriched"
            ...
            for json_file in sorted(new_menus_dir.glob("discovery_*.json")):
                try:
                    ...
                    for menu in data.get("menus", []):
                        name_ko = menu.get("name_ko", "").replace("'", "''")
                        name_en = (menu.get("name_en", "") or "").replace("'", "''")
                        source = menu.get("source", "automation")
                        category = menu.get("category_hint", "")
        ...
        # 2. Enriched 콘텐츠 UPDATE
        if enriched_dir.exists():
            for json_file in sorted(enriched_dir.glob("enrichment_batch_*.json")):
                try:
                    with open(json_file, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    ...
                    for menu in data.get("menus", []):
                        ...
                        desc_ko = (content.get("description_ko", "") or "").replace("'", "''")
                        desc_en = (content.get("description_en", "") or "").replace("'", "''")
                        ...
                        sql_lines.append(
                            ...
                            f"regional_variants = '{json.dumps(content.get('regional_variants', []), ensure_ascii=False).replace(chr(39), chr(39)+chr(39))}' "
                            f"WHERE name_ko = '{name_ko}';"
                        )
