...
# 브랜드 접두사 (제거 후 재평가)
BRAND_PREFIXES = [
    "농심", "오뚜기", "삼양", "풀무원", "CJ", "비비고", "동원",
    "해찬들", "청정원", "백설", "진라면", "신라면", "불닭",
]

# 매장/프랜차이즈 접두사 (제거 후 재평가)
STORE_PREFIXES = [
    "마성", "말죽거리", "소식좌", "교동", "명인", "맛남",
]

# 무효 접미사 (세트/정식/곱배기는 변형이지 새 메뉴가 아님)
INVALID_SUFFIXES = [
    "세트", "정식", "곱배기", "만들기", "만드는법", "레시피",
    "만들기♪", "만들기♪♪",
]

# 개념어/비음식 (메뉴가 아닌 카테고리 또는 개념)
NON_MENU_KEYWORDS = [
    "요리", "향토음식", "cuisine", "regional",
    "culture", "history", "food culture",
]
...
def filter_menu_name(name: str) -> Optional[str]:
    ...
    if len(cleaned) > 10:
        cooking_words = ["볶음", "무침", "조림", "구이", "찜", "전", "탕", "국", "찌개", "밥"]
