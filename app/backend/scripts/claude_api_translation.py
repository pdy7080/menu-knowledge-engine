...
def build_translation_prompt(menus: List[Dict]) -> str:
    ...
    for m in menus:
        ...
        menu_list.append({
            ...
            "description_en": expl_en or f"Korean dish: {m.get('name_en', m['name_ko'])}"
        })
...
def update_translation(conn, menu_id: str, translation: Dict) -> bool:
    ...
    try:
        ...
        cur.execute(
            "SELECT explanation_short FROM canonical_menus WHERE id = %s",
            (menu_id,)
        )
        row = cur.fetchone()
        current_expl = dict(row["explanation_short"]) if row and row["explanation_short"] else {}
...
def main():
    ...
    try:
        ...
        logger.info(f"[API] Claude API 초기화 완료\n")
        ...
        for batch_idx in range(0, total, batch_size):
            ...
            try:
                ...
                logger.debug(f"  응답 텍스트 파싱 실패")
                failed += len(batch)
            except anthropic.RateLimitError:
                logger.warning(f"  [RATE LIMIT] 60초 대기 후 재시도...")
                time.sleep(60)
                # 재시도
                try:
                    translations = translate_batch(client, batch)
                    for menu in batch:
                        menu_id = menu["id"]
                        if menu_id in translations:
                            ok = update_translation(conn, menu_id, translations[menu_id])
                            if ok:
                                success += 1
                            else:
                                failed += 1
        ...
        logger.info(f"  전체 번역 완료율: {stats_after['rate']} ({stats_after['translated']}/{stats_after['total']})")
        logger.info("=" * 60)
