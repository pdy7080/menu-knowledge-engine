"""
300개 메뉴 콘텐츠 전체 확장 - V2 Production Script
...
"""
    ...
    async def save_checkpoint(self, count: int):
        """체크포인트 저장"""
        checkpoint_path = BASE_DIR.parent.parent / "data" / f"checkpoint_{count}.json"
...
async def main():
    """메인 함수"""
    print("="*60)
    print("GPT-4o 콘텐츠 전체 확장 V2 - 300개 메뉴")
    print("Team Lead 승인 (2026-02-19)")
    print("="*60)
    ...
    # 결과 저장
    output_path = BASE_DIR.parent.parent / "data" / "enriched_menus_full_v2.json"
    ...
    print("최종 통계")
    print("="*60)
    print(f"처리된 메뉴: {enricher.enriched_count + enricher.failed_count}/300")
    print(f"성공: {enricher.enriched_count}개 ({enricher.enriched_count / 300 * 100:.1f}%)")
    print(f"실패: {enricher.failed_count}개")
    print(f"\n다음 단계: 품질 검증")
    print(f"  python app/backend/scripts/validate_enriched_content.py {output_path}")
