...
async def enrich_menu_with_gemini(menu_name_ko: str, menu_name_en: str) -> Dict[str, Any]:
...
async def enrich_all_menus(max_retries=3):
    ...
    for idx, menu in enumerate(menus, 1):
        ...
        for attempt in range(max_retries):
            ...
            if enriched:
                ...
                print(f"[{idx:3d}/{len(menus)}] ✗ {menu_name_ko} (failed after {max_retries} attempts)")
        ...
        if idx % 10 == 0:
            ...
            # 체크포인트 저장
            checkpoint_file = BASE_DIR / "data" / f"enriched_menus_checkpoint_{idx}.json"
    ...
    print(f"[RESULT] 성공: {success_count}/{len(menus)} ({success_count/len(menus)*100:.1f}%)")
    print(f"[FAILED] {len(failed_menus)}개 실패: {', '.join(failed_menus[:5])}{'...' if len(failed_menus) > 5 else ''}")
