"""
Sprint 2 Phase 2 P1: 나머지 149개 메뉴 콘텐츠 확장
2026-02-19
...
Based on: enrich_content_full_v2.py
"""
import asyncio
import json
import sys
...
class ContentEnricher:
    ...
    async def enrich_menu(self, menu: CanonicalMenu, retry_count: int = 0) -> Optional[Dict[str, Any]]:
        """단일 메뉴 콘텐츠 생성 (retry 지원)"""
...
Output JSON with 8 fields: description_ko, description_en, regional_variants, preparation_steps, nutrition, flavor_profile, visitor_tips, similar_dishes, cultural_background
"""
            ...
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a Korean food historian. Provide accurate, menu-specific information. Output ONLY valid JSON."
                    },
            ...
            if retry_count < 3:
                ...
                return await self.enrich_menu(menu, retry_count + 1)
            else:
                print(f"  [FAIL] {menu_name}: {str(e)[:50]}")
                self.failed_count += 1
                self.errors.append({
                    "menu_id": str(menu.id),
                    "name_ko": menu_name,
    ...
    async def enrich_remaining_menus(self, limit: int = 200) -> List[Dict[str, Any]]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            stmt = select(CanonicalMenu).where(
                ...
                CanonicalMenu.description_long_ko == None  # enriched content 없음
            ).limit(limit)
            result = await session.execute(stmt)
            menus = list(result.scalars().all())

            total = len(menus)
            print(f"\n총 {total}개 메뉴 처리 시작 (enriched content 없는 메뉴만)...")
            print(f"모델: {self.model} (temperature={self.temperature})")
            print(f"카테고리: 6개 (stew, soup, grilled, stir-fried, rice, noodles)")
            print("="*60)
            ...
            for i in range(0, total, batch_size):
                ...
                print(f"\n[{i + batch_size}/{total}] 진행: {progress:.1f}% | 성공: {self.enriched_count} | 실패: {self.failed_count}")

                # 체크포인트 저장 (10개마다)
                if (i + batch_size) % (self.checkpoint_interval * batch_size) == 0:
                    await self.save_checkpoint(i + batch_size)
    ...
    async def save_checkpoint(self, count: int):
        """체크포인트 저장"""
        checkpoint_path = BASE_DIR.parent.parent / "data" / f"checkpoint_remaining_{count}.json"
...
async def main():
    """메인 함수"""
    print("="*60)
    print("Sprint 2 Phase 2 P1: 나머지 149개 메뉴 콘텐츠 확장")
    print("2026-02-19")
    print("="*60)
    ...
    # 결과 저장
    output_path = BASE_DIR.parent.parent / "data" / "enriched_remaining_149_menus.json"
    ...
    if enricher.enriched_count + enricher.failed_count > 0:
        print(f"성공률: {enricher.enriched_count / (enricher.enriched_count + enricher.failed_count) * 100:.1f}%")
    print(f"\n다음 단계: DB 업데이트")
