...
# 1. 필수 메뉴 확인
required_menus = {
    "비빔밥": ("비빔밥", "Bibimbap (Mixed Rice with Vegetables)", "밥에 여러 가지 나물과 고기를 얹어 비벼 먹는 요리"),
    "냉면": ("냉면", "Naengmyeon (Cold Noodles)", "차가운 육수에 메밀면을 말아 먹는 여름 별미"),
    "칼국수": ("칼국수", "Kalguksu (Hand-cut Noodle Soup)", "칼로 썬 밀가루 면을 국물에 넣어 끓인 요리"),
    ...
    "김밥": ("김밥", "Gimbap (Seaweed Rice Roll)", "김에 밥과 여러 재료를 싸서 만 요리"),
}
...
# 빈도순 정렬
sorted_core_menus = sorted(core_menus.items(), key=lambda x: x[1]["count"], reverse=True)
...
for name_ko, name_en, desc_ko in missing_required:
    concept = "비빔밥" if "비빔" in name_ko else \
              "냉면" if "냉면" in name_ko else \
              "칼국수" if "칼국수" in name_ko else \
              "면류" if "국수" in name_ko or "면" in name_ko else \
              "튀김" if "까스" in name_ko else \
              "육류안주" if "육회" in name_ko else \
              "밥류" if "김밥" in name_ko else "기타"
    ...
    menus_to_add.append({
        ...
        "description_en": name_en.split("(")[1].replace(")", "") if "(" in name_en else name_en,
        "primary_ingredients": [],
        "allergens": [],
        "spice_level": 0,
        "difficulty_score": 1,
...
for menu in menus_to_add[:20]:  # 샘플로 20개만 출력
    ...
    print(f"            \"primary_ingredients\": [],")
    print(f"            \"allergens\": [],")
    print(f"            \"spice_level\": 0,")
    print(f"            \"difficulty_score\": 1,")
    print(f"        }},")
