...
async def ingest_public_data(limit: int = 0, dry_run: bool = False, force: bool = False):
    """공공데이터 일괄 임포트"""
    print(f"[START] 공공데이터 일괄 임포트")
        ...
        for idx, menu in enumerate(menus, 1):
            ...
            for variant in variants:
                try:
                    ...
                    if enriched and (enriched.get("nutrition_info") or enriched.get("standard_code")):
                        break
                except Exception as e:
                    print(f"\n  [WARN] API error for '{variant}': {e}")
                    continue

            if not enriched or (not enriched.get("nutrition_info") and not enriched.get("standard_code")):
                failed += 1
    ...
    print("-" * 60)
    print(f"[COMPLETE] 처리 완료")
    print(f"[RESULT] OK={success}, FAIL={failed}, SKIP={skipped}")
    if failed_names:
        print(f"[FAILED] {', '.join(failed_names[:10])}{'...' if len(failed_names) > 10 else ''}")
    print(f"[TIME] {datetime.now().isoformat()}")
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="이미 데이터 있는 메뉴도 재처리")
