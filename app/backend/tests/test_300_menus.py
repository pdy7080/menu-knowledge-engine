...
RESULT_PATH = Path(__file__).parent.parent.parent / "data" / "matching_test_results.json"
...
async def test_300_menus():
    ...
    stats = {
        ...
        "by_frequency": {
            "상": {"total": 0, "exact": 0, "modifier_decomposition": 0, "ai_discovery_needed": 0},
            "중": {"total": 0, "exact": 0, "modifier_decomposition": 0, "ai_discovery_needed": 0},
            "하": {"total": 0, "exact": 0, "modifier_decomposition": 0, "ai_discovery_needed": 0},
        },
        "failed_cases": [],
        "all_results": []
    }
            ...
            try:
                ...
                if match_type == "ai_discovery_needed":
                    stats["failed_cases"].append({
                        ...
                        "modifiers_found": [m["text_ko"] for m in result.get("modifiers", [])],
                        "notes": item["notes"]
                    })
                ...
                stats["all_results"].append({
                    ...
                    "canonical": result.get("canonical", {}).get("name_ko") if result.get("canonical") else None,
                    "modifiers": [m["text_ko"] for m in result.get("modifiers", [])],
    ...
    print(f"\n1. 전체 매칭률 ({total}개)")
    print(f"   - exact_match:             {exact_count:3d}개 ({exact_count/total*100:5.1f}%)")
    print(f"   - modifier_decomposition:  {modifier_count:3d}개 ({modifier_count/total*100:5.1f}%)")
    print(f"   - similarity:              {similarity_count:3d}개 ({similarity_count/total*100:5.1f}%)")
    print(f"   - ai_discovery_needed:     {ai_needed_count:3d}개 ({ai_needed_count/total*100:5.1f}%)")
    ...
    for freq in ["상", "중", "하"]:
        ...
        print(f"   - {freq}(기본형): {freq_success}/{freq_total}개 ({freq_success_rate:.1f}%)")
        print(f"       exact: {freq_stats['exact']}, modifier: {freq_stats['modifier_decomposition']}, ai_needed: {freq_stats['ai_discovery_needed']}")
