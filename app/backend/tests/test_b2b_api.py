...
async def test_b2b_api():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            response = await client.post(f"{BASE_URL}/api/v1/b2b/restaurants", json=payload)
            ...
            if response.status_code == 200:
                ...
                if data.get("success") and data.get("restaurant_id"):
                    ...
                    if data.get("status") == "pending_approval":
                        ...
                        results.add_fail("Default status check", f"Expected pending_approval, got {data.get('status')}")

                    if "approval_url" in data:
                        results.add_pass("Approval URL provided")
                    else:
                        results.add_fail("Approval URL check", "approval_url not in response")
                else:
                    results.add_fail("Registration response format", f"Missing success or restaurant_id: {data}")
            else:
                results.add_fail("Restaurant registration", f"Status code: {response.status_code}, Body: {response.text}")
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                results.add_fail("Duplicate check", f"Expected 400, got {response.status_code}")
        ...
        if results.restaurant_id:
            ...
            try:
                response = await client.get(f"{BASE_URL}/api/v1/b2b/restaurants/{results.restaurant_id}")
                ...
                if response.status_code == 200:
                    ...
                    if data.get("id") == results.restaurant_id:
                        ...
                        results.add_fail("Restaurant ID check", f"Expected {results.restaurant_id}, got {data.get('id')}")

                    if data.get("name") == payload["name"]:
                        results.add_pass("Restaurant name matches")
                    else:
                        results.add_fail("Restaurant name check", f"Expected {payload['name']}, got {data.get('name')}")

                    if data.get("business_license") == payload["business_license"]:
                        results.add_pass("Business license matches")
                    else:
                        results.add_fail("Business license check", f"Expected {payload['business_license']}, got {data.get('business_license')}")

                else:
                    results.add_fail("Get restaurant info", f"Status code: {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                if "total" in data and "data" in data:
                    ...
                    if data["total"] > 0:
                        ...
                        results.add_fail("Restaurant count", "Expected at least 1 restaurant")

                else:
                    results.add_fail("List response format", f"Missing total or data: {data}")

            else:
                results.add_fail("List restaurants", f"Status code: {response.status_code}")
        ...
        try:
            response = await client.get(f"{BASE_URL}/api/v1/b2b/restaurants?status=pending_approval")

            if response.status_code == 200:
                data = response.json()

                all_pending = all(r["status"] == "pending_approval" for r in data.get("data", []))
                if all_pending:
                    results.add_pass("Status filter works correctly")
                else:
                    results.add_fail("Status filter", "Some restaurants have different status")

            else:
                results.add_fail("Filter by status", f"Status code: {response.status_code}")
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success"):
                        ...
                        if data.get("status") == "active":
                            ...
                            results.add_fail("Status after approval", f"Expected active, got {data.get('status')}")

                    else:
                        results.add_fail("Approval response", f"success=False: {data}")

                else:
                    results.add_fail("Approve restaurant", f"Status code: {response.status_code}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") == "active":
                        ...
                        results.add_fail("Status verification", f"Expected active, got {data.get('status')}")
                    ...
                    if data.get("approved_by") == "test_admin_001":
                        ...
                        results.add_fail("approved_by check", f"Expected test_admin_001, got {data.get('approved_by')}")
        ...
        try:
            ...
            response = await client.post(f"{BASE_URL}/api/v1/b2b/restaurants", json=reject_payload)
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") == "rejected":
                        ...
                        results.add_fail("Rejection status", f"Expected rejected, got {data.get('status')}")

                    # Verify rejection
                    response = await client.get(f"{BASE_URL}/api/v1/b2b/restaurants/{reject_id}")
                    data = response.json()

                    if data.get("rejection_reason") == reject_request["rejection_reason"]:
                        results.add_pass("Rejection reason saved correctly")
                    else:
                        results.add_fail("Rejection reason", f"Expected '{reject_request['rejection_reason']}', got '{data.get('rejection_reason')}'")

                else:
                    results.add_fail("Reject restaurant", f"Status code: {response.status_code}")
        ...
        if results.restaurant_id:
            ...
            try:
                ...
                if response.status_code == 400:
                    ...
                    results.add_fail("Invalid action check", f"Expected 400, got {response.status_code}")
        ...
        try:
            response = await client.get(f"{BASE_URL}/api/v1/b2b/restaurants?limit=1&offset=0")
            ...
            if response.status_code == 200:
                ...
                if len(data.get("data", [])) <= 1:
                    ...
                    if data.get("limit") == 1 and data.get("offset") == 0:
                        ...
                        results.add_fail("Pagination metadata", f"limit={data.get('limit')}, offset={data.get('offset')}")

                else:
                    results.add_fail("Pagination limit", f"Expected 1 item, got {len(data.get('data', []))}")
