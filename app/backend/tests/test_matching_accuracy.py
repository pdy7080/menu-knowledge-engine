...
from services.matching_engine import MenuMatchingEngine  # noqa: E402
from tests.test_cases.exact_match import EXACT_MATCH_CASES  # noqa: E402
from tests.test_cases.normalization import NORMALIZATION_CASES  # noqa: E402
from tests.test_cases.suffix_patterns import SUFFIX_CASES  # noqa: E402
from tests.test_cases.modifier_decomposition import MODIFIER_CASES  # noqa: E402
from tests.test_cases.similarity_match import SIMILARITY_CASES  # noqa: E402

# Requires live PostgreSQL connection — skip in local environments without DB
pytestmark = pytest.mark.skip(
    reason="Requires PostgreSQL connection (init_db fails without DB)"
)
    ...
    async with AsyncSessionLocal() as db_session:
        ...
        if failed_cases:
            ...
            for i, case in enumerate(failed_cases[:20], 1):
                print(f"\n  {i}. 입력: '{case['input']}'")
                print(
                    f"     기대: {case['expected_type']} → {case['expected_canonical']}"
                )
                ...
                if "error" in case:
                    ...
                    print(
                        f"     실제: {case.get('actual_type', 'N/A')} → {case.get('actual_canonical', 'N/A')}"
                    )
...
@pytest.mark.asyncio
async def test_individual_exact_match():
    """개별 테스트: 정확 매칭"""
    await init_db()
    async with AsyncSessionLocal() as db_session:
        engine = MenuMatchingEngine(db_session)
        ...
        for input_text, expected_canonical, expected_type in EXACT_MATCH_CASES:
            ...
            if (
                result.canonical
                and result.canonical.get("name_ko") == expected_canonical
            ):
                passed += 1
...
@pytest.mark.asyncio
async def test_individual_normalization():
    """개별 테스트: 정규화"""
    await init_db()
    async with AsyncSessionLocal() as db_session:
        engine = MenuMatchingEngine(db_session)
        ...
        accuracy = (
            (passed / len(NORMALIZATION_CASES) * 100) if NORMALIZATION_CASES else 0
        )
...
@pytest.mark.asyncio
async def test_individual_modifier_decomposition():
    """개별 테스트: 수식어 분해"""
    await init_db()
    async with AsyncSessionLocal() as db_session:
        engine = MenuMatchingEngine(db_session)
