"""
Gemini REST API 콘텐츠 강화 스크립트 (requests 사용)
시드 데이터 → Gemini REST API → 112 menus 콘텐츠 생성
...
5. No markdown formatting, just pure JSON"""
    ...
    def _validate_content(self, content: Dict[str, Any]) -> bool:
        ...
        if len(content.get("regional_variants", [])) < 3:
            print(f"  ⚠️  regional_variants 부족")
            return False
        if len(content.get("preparation_steps", [])) < 5:
            print(f"  ⚠️  preparation_steps 부족")
            return False
        if len(content.get("similar_dishes", [])) < 3:
            print(f"  ⚠️  similar_dishes 부족")
            return False
    ...
    def enrich_all_menus(self, menus: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        print(f"모델: {self.model_name} (REST API)")
        print(f"무료 tier: 15 RPM")
        print("="*60)
...
def main():
    """메인 함수"""
    print("="*60)
    print("Gemini 2.0 Flash 콘텐츠 강화 (REST API)")
    print("="*60)
